---
description: >-
  Understand how Harness Resilience Testing detects risks, scores them by
  severity, and confirms them with chaos experiments.
tags:
  - chaos-engineering
  - risks
---
...
## Where risks come from <a href="#where-risks-come-from" id="where-risks-come-from"></a>

Risks are not entered by hand. Every risk is the output of a **scan**. Scans run from three places.

* **Onboarding scan:** The **Scanning** stage of Resilience Testing service onboarding assesses the workloads you select before they become services. Go to [Service discovery and onboarding](../services/service-discovery.md#scanning-stage) for that flow.
...
* **Infrastructure scan:** Reads the applications a [discovery agent](https://developer.harness.io/harness-platform/use-harness-platform/service-discovery) already found in a cluster. Each scan maps to one discovery agent.

All three feed the same analysis, so the risks and reports look the same whichever entry point you use. Go to [Risk scans](risk-scans.md) to run a pipeline or infrastructure scan on demand.

{% hint style="info" %}
**RISKS DESCRIBE CONFIGURATION, NOT BEHAVIOR**
...
## Risk rules <a href="#risk-rules" id="risk-rules"></a>
...
## Recommendations and mitigation <a href="#recommendations-and-mitigation" id="recommendations-and-mitigation"></a>
...
{% hint style="info" %}
**FINDINGS REFLECT THE MANIFEST, NOT THE LIVE CLUSTER**
...
## Severity and risk score <a href="#severity-and-risk-score" id="severity-and-risk-score"></a>
...
{% hint style="info" %}
**A SCORE OF 500 IS THE RECOMMENDED GATE THRESHOLD**
...
### Resilience Risk Score <a href="#resilience-risk-score" id="resilience-risk-score"></a>
...
## The risk heat map <a href="#the-risk-heat-map" id="the-risk-heat-map"></a>

The **Risk Detection Heatmap** on a scan is a matrix of services against risk rules, colored by severity. A cell marked **No risk detected** means the rule produced nothing for that service, either because the service satisfies it or because the rule does not apply.

![Risk Detection Heatmap with services as rows and risk rules as columns, each cell colored by severity from Critical through Low, and gray cells where no risk was detected](../../.gitbook/assets/risk-detection-heatmap.png)
...
## Passive and confirmed risks <a href="#passive-and-confirmed-risks" id="passive-and-confirmed-risks"></a>
...
{% hint style="info" %}
**TREAT PASSIVE RISKS AS A TEST BACKLOG**
...
### Associate a risk rule with a probe <a href="#associate-a-risk-rule-with-a-probe" id="associate-a-risk-rule-with-a-probe"></a>
...
## Risks and services <a href="#risks-and-services" id="risks-and-services"></a>
...
## Review risks across the project <a href="#review-risks-across-the-project" id="review-risks-across-the-project"></a>
...
**Summary** gives you the totals. **Passive Risks** splits the count by what was scanned, so risks found by scanning pipeline metadata are counted separately from risks found by scanning services metadata, each broken down by severity. **Confirmed Risks** counts the risks validated by probe failures and links to **View Confirmed Risks**. Below the totals, **Services with risk** ranks every affected service from highest to lowest severity composition, with a stacked **Risk Composition** bar per service.

![Summary tab of the Risks page, with Passive Risks split into a pipeline metadata donut and a services metadata donut, each broken down by severity, beside a Confirmed Risks card](../../.gitbook/assets/risks-summary-tab.png)

**Risks** is the full list, one row per risk per service, with a **Severity**, **Risk**, **Service**, **Latest Validation**, and **Last Detected** column, sorted by **Severity (High → Low)**. Filter it by **Severity**, **Service ID** (the immutable service identifier), **Validation**, or **Source**, and use **Search for a risk** to narrow it by rule name.

### Open a single risk <a href="#open-a-single-risk" id="open-a-single-risk"></a>

Select a risk to open its detail page, which is headed with the severity, the rule name, and the service the risk was found in. The page has two tabs.

**Overview** holds **About this Risk**, which names the **Risk name**, the **Affected Service**, the **Severity**, the **Latest Validation** state, and a **Description** of what the rule detects.

**Detection Log** lists every time the risk has been reported, under a **Total Detections** count, with these columns.
...
Filter the detections by **Source**, **Validation**, or **Timeframe**, which defaults to **All time** on this tab rather than the last 30 days. Because each detection names its scan and source, you can tell a risk that one scan found once from a risk every scan since has reported.

Open a single service to review only the risks recorded against it.

***

## Availability on Self-Managed Enterprise Edition <a href="#availability-on-self-managed-enterprise-edition" id="availability-on-self-managed-enterprise-edition"></a>
...
* [Risk scans](risk-scans.md): Run a pipeline scan or an infrastructure scan and read the report.
* [Probes](../../chaos-testing/probes/): Build the checks that confirm a risk.
* [Chaos experiments](../../chaos-testing/experiments/): Run a fault to verify a risk against a live system.
