---
description: >-
  Openshift Security Context Constraint to control permissions for pods in
  cluster
---
...
{% hint style="info" %}
You can leverage all the [permissions mentioned](openshift-scc.md#run-service-account-as-a-cluster-admin) for fault execution as well as [service discovery](https://developer.harness.io/harness-platform/use-harness-platform/service-discovery/user-defined-service-account). The SCC described below is a superset, which means only some of them are required for service discovery.
{% endhint %}
