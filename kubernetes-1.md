...
### Delegate Architecture <a href="#delegate-architecture" id="delegate-architecture"></a>
...
There a several ways to deploy the delegate [documented here](https://app.gitbook.com/s/3F2TpHXhur2QtQnORSM9/use-harness-platform/delegates/delegate/install-delegates/overview), but the recommended approach is to use Helm.
...
When deploying a delegate, it is recommended that you name the delegate either the same as the cluster name or something very similar that makes it obvious what cluster the delegate is deployed into. An example of deploying a CACM Only delegate via a Helm chart is [located here](https://app.gitbook.com/s/LgkNK1xq1Uj6rzIm2kum/knowledge-base-and-reference-architecture/best-practices/k8s/ccm-only-delegate).

#### Resource Constraints <a href="#resource-constraints" id="resource-constraints"></a>

* The delegate pod needs a minimum of 1 vCPU and 2G of memory. For larger clusters, refer to the sizing table in the Deployment section above.
...
#### Kubernetes Connector <a href="#kubernetes-connector" id="kubernetes-connector"></a>
...
If choosing to deploy a delegate inside of the cluster, you will specify the delegate name that corresponds to the delegate that should be used to make connections for the cluster. [There is an in-depth guide here](https://app.gitbook.com/s/3F2TpHXhur2QtQnORSM9/use-harness-platform/connectors/cloud-providers/add-a-kubernetes-cluster-connector).
...
```terraform
# when using a delegate deployed into the cluster <a href="#when-using-a-delegate-deployed-into-the-cluster" id="when-using-a-delegate-deployed-into-the-cluster"></a>
...
#### CACM Kubernetes Connector <a href="#cacm-kubernetes-connector" id="cacm-kubernetes-connector"></a>
...
### Enable Auto Stopping <a href="#enable-auto-stopping" id="enable-auto-stopping"></a>
...
### Service Account for CACM <a href="#service-account-for-cacm" id="service-account-for-cacm"></a>
...
{% hint style="danger" %}
#### Not supported for CACM

Creating a Kubernetes connector using a master URL and credentials (without a delegate inside the cluster) is not supported for CACM. CACM requires a delegate deployed directly into each target cluster to collect metrics. Go to Kubernetes for the supported setup.
{% endhint %}

### Using a pre-existing Harness account <a href="#using-a-pre-existing-harness-account" id="using-a-pre-existing-harness-account"></a>
