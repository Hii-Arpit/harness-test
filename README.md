...
* **Recommendation Type**: CACM has six types of recommendations: `AZURE_INSTANCE`, `EC2_INSTANCE`, `GOVERNANCE`, `NODE_POOL` and `WORKLOAD`. See details about all the types of recommendations here.
* **Cloud Provider**:
* **+ Add Filter**
...
**2. Actual Savings Field Mapping** Map Jira custom fields to capture actual savings from applied recommendation tickets. When tickets reach the statuses configured in Section 1, the savings values from your Jira custom fields are synced to CACM. For each mapping, you need to specify:
...
Workload costs for GCP are fixed at **List price** and cannot be changed.
{% endtab %}
