---
description: >-
  This article addresses some frequently asked questions about Harness Chaos
  Engineering.
---
...
For an overview of Harness support for platforms, methodologies, and related technologies, go to [Supported platforms and technologies](https://developer.harness.io/harness-platform/new-to-harness-platform/platform-whats-supported).
...
Git-based ChaosHubs have been removed. Connecting to private Git-based hubs is no longer supported. Use [Templates](../chaos-testing/templates/) to manage reusable chaos artifacts instead.
{% endhint %}
...
![Resilience Score](../.gitbook/assets/chaos-engineering-faq-resilience-score.png)
...
## Features and capabilities <a href="#features-and-capabilities" id="features-and-capabilities"></a>

<details>

<summary><strong>How do you inject chaos on managed cloud services?</strong></summary>

You can obtain the required permissions to inject chaos on respective cloud-services like [AWS permissions](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/permissions), [Cloud Foundry permissions](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/cloud-foundry/permissions), and [GCP](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/gcp/security-configurations/prepare-secret-for-gcp).
...
Yes. For more information, go to [AWS chaos](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/aws), [GCP chaos](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/gcp/gcp), and [Azure chaos](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/azure/azure).
...
Yes. For more information, go to [ALB zone down](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/alb-az-down), [CLB zone down](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/clb-az-down), and so on.
...
Yes. For more information, go to [pod memory hog](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/kubernetes/pod/pod-memory-hog) and [pod CPU hog](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/kubernetes/pod/pod-cpu-hog).
...
Yes. For more information, go to [locust loadgen](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/load/locust-loadgen).
...
Yes, you can use [SSH chaos](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/ssh/ssh-chaos) to inject chaos on switches, load balancers, and so on.
...
Yes, you can use set the `DESTINATION_IPS` or `DESTINATION_HOSTS` tunables in all network level chaos experiments. For example, to execute the network loss fault,
...
## Harness Delegate <a href="#harness-delegate" id="harness-delegate"></a>
...
You can access the Harness control plane from the user environment with outbound connectivity over HTTPS using port 443. Go to [permissions and ports](https://developer.harness.io/harness-platform/use-harness-platform/references/permissions-and-ports-for-harness-connections) for more details.
...
* [Kubernetes permissions](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/kubernetes/tkgi/permissions);
* [AWS permissions](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/permissions); and
* [Linux permissions](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/linux/permissions), respectively.
...
Yes, with cloud-based faults, chaos pods can be mapped to specific IAM roles. You have to create the IAM role on the cloud account and map it to the appropriate policy. The user's credentials should be embedded in a K8s secret before executing the faults. You can also create [a superset AWS policy](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/security-configurations/policy-for-all-aws-faults) to allow executing all fault types. For more information, go to [AWS switch profile](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/security-configurations/aws-switch-profile) and [workload identity setup for GCP](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/gcp/security-configurations/gcp-iam-integration).
...
Yes, Harness CE has dedicated agents for different target types such as Linux, Windows, Kubernetes, and Cloud platforms (AWS, Azure, GCP, Cloud Foundry). The agents are:
...
Go to [prerequisites](on-premise-vs-saas.md) to fulfill the requirements before onboarding. Once all the prerequisites are fulfilled, you can explore [sandbox](training-and-certification/sandbox.md) or execute [your first chaos experiment](on-premise-vs-saas.md).

**OR**

If you want a head start to your journey with Harness CE, you can onboard Harness CE in two ways:

* [Automated onboarding](../chaos-testing/get-started.md); and
* [Guided onboarding](../chaos-testing/get-started.md).
...
* [Run chaos experiments](on-premise-vs-saas.md)
...
The sandbox onboarding UI has been removed. Go to [Get Started](../chaos-testing/get-started.md) to run your first chaos experiment.
...
## Kubernetes experiment flow optimization <a href="#kubernetes-experiment-flow-optimization" id="kubernetes-experiment-flow-optimization"></a>
...
To create a new experiment, you need to have at least one infrastructure in version 1.38.x or higher. Hence, you can either [connect a new infrastructure](../chaos-testing/infrastructure/) or [upgrade an existing one](../chaos-testing/infrastructure/linux/upgrade.md).
...
## Application Maps <a href="#application-maps" id="application-maps"></a>

<details>

<summary><strong>How to manually associate experiments as a part of Application Map?</strong></summary>

To manually associate the experiment as a part of an application map, list the experiment as a part of an [application map](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/application-maps), specify the tag `applicationmap=<application map identity>` while creating the experiment.
...
The Harness platform is fully integrated with several public OAuth providers, with support for two-factor authentication and domain whitelisting. To learn more, go to [authentication overview](https://developer.harness.io/harness-platform/use-harness-platform/authentication).
...
The chaos infrastructure connects to the Harness control plane through outbound connectivity over HTTP(s) using port 443. Go to [Chaos infrastructures](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/infrastructure/infrastructures) to read how the chaos infrastructure connects to Harness.
...
The chaos infrastructure setup involves the creation of CRDs and RBAC resources. This setup typically needs cluster-admin intervention. To learn more, go to [Kubernetes roles for the chaos infrastructure](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/security/index#kubernetes-roles-for-chaos-infrastructure).
...
The scope of a user's access to chaos resources added to a given Harness account or project can be controlled by assigning them a predefined or custom role. Go to [chaos access control](../shared-capabilities/rbac.md) to assign roles.
...
In addition, users can provide a custom service account to carry out experiments, thereby limiting the fault types in the user environment. To learn more, go to [blast radius control using permissions](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/security/index#blast-radius-control-using-permissions).
...
Harness Chaos experiment pods consume Kubernetes secrets that contain access credentials, which are leveraged to make provider-specific API calls to the cloud platform to inject chaos. To learn more, go to [Secrets management](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/security/index#secrets-management).
...
Yes. Chaos experiments can resolve secrets from an external secret manager the same way they resolve secrets from the Harness Secret Manager, so your existing experiments and probes do not change. This is enabled by the dedicated chaos delegate task and is behind the feature flag `CHAOS_DEDICATED_DELEGATE_TASK`. Support has been validated with HashiCorp Vault and Google Cloud KMS. Go to [Use an external secret manager](../security/index.md#use-an-external-secret-manager) to understand how it works.
...
When the chaos infrastructure is deployed on EKS clusters, the experiments can leverage the IAM service account (IRSA) instead of consuming secrets with user account access details. To learn more, go to [IAM integration for AWS authentication](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/security-configurations/aws-iam-integration).
...
Harness Chaos experiments can consume K8s secrets containing authentication information for the desired APM and use it within the command-probe pods that leverage this information to make the right provider-specific API calls to retrieve metrics and other pertinent data. To learn more, go to [command probes](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/probes/command-probes/index).
...
Harness provides an audit log to the account admin where user actions on the chaos resources are logged with timestamps. To learn more, go to [audit trail](https://developer.harness.io/harness-platform/use-harness-platform/governance/audit-trail/audit-trail).
...
Yes, [chaos dedicated infrastructure](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/infrastructure/types/legacy-infra/legacy-infra) as well as [Harness Delegate](https://www.harness.io/blog/delegate-installation-via-helm) can be installed using Helm charts.
...
Yes, Harness CE provides integration with many tools, such as [Gitlab pipelines](../shared-capabilities/integrations/cicd/gitlab-ci.md), [Jenkins pipelines](../shared-capabilities/integrations/cicd/jenkins.md), with [Harness CD](../shared-capabilities/integrations/cicd/harness-cd.md), Harness Feature Flags, and SRM.
...
Yes, you can use resilience probes with [Prometheus](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/probes/prometheus-probes/index), [Dynatrace](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/probes/apm-probes/index), [DataDog](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/probes/apm-probes/index), and [NewRelic](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/probes/command-probes/index).
