...
* **AWS connector:** A configured AWS connector in Harness using either CUR 1.0 or CUR 2.0. Both connector types surface account tags, but with different historical behavior. Navigate to [Set up cost visibility for AWS](/broken/pages/a3cd177065b04428c1008c82f0d2c3585269bfe1) to configure your connector.
...
{% tabs %}
{% tab title="CUR 2.0" %}
Account-level tags are available directly in the CUR 2.0 report data. Tags flow from the AWS Organizations management account into each month's CUR export under the `resource_tags` field, with the `accountTag/` prefix. Harness ingests them into `Label V2` alongside that month's cost data.

**Tag update behavior:** Tags are versioned per month. If you change an account tag today, previous months keep the tags they had when that billing data was exported. Past data does not change retroactively.

**Migrating from CUR 1.0?** Once you migrate your connector to CUR 2.0, account tags are reflected automatically from the CUR data. No API sync is needed.
{% endtab %}

{% tab title="CUR 1.0" %}
AWS does not include account-level tags in the CUR 1.0 report. Harness fetches them directly from the AWS Organizations API instead. Harness stores the latest tags in a lookup table daily and joins that table against the unified cost table at query time.

**Tag update behavior:** Only the latest tag snapshot is kept, with no versioning. Querying any historical month returns the tags as they exist today. Tag changes are reflected across all historical data within 24 hours.
{% endtab %}
{% endtabs %}
...
* **Billing export:** Project labels are included automatically from your GCP billing export. No additional Harness configuration is required. Go to [Set up CACM for GCP](/broken/pages/a5bd2aa828fd74c29077baf1c09ec2e812f57b86) to configure your connector.
...
* **Tag inheritance:** Subscription and resource group tags are not visible in Harness until you enable tag inheritance in Azure. Go to the [Enable tag inheritance](/broken/pages/d1ec31149b8608a6db2a8678a49f0bb2900f6365#enable-tag-inheritance) steps below.
* **Azure connector:** A configured Azure connector in Harness. After you enable tag inheritance, the connector picks up the tags automatically on its next ingestion cycle. Go to [Set up cost visibility for Azure](/broken/pages/a5bd2aa828fd74c29077baf1c09ec2e812f57b86) to configure your connector.
...
**Set the conflict policy:** You choose the conflict policy on the Azure **Tag inheritance** screen when you enable inheritance. Go to [Enable tag inheritance](/broken/pages/d1ec31149b8608a6db2a8678a49f0bb2900f6365#enable-tag-inheritance) to configure it.
...
{% stepper %}
{% step %}
In the Azure portal, go to **Cost Management** and select **Manage subscription**.
{% endstep %}
...
{% step %}
Enable tag inheritance for each subscription or billing scope.
{% endstep %}

{% step %}
Choose your conflict behavior:
...
{% step %}
Wait for the next ingestion cycle to pick up the newly added or enabled tags.
{% endstep %}
{% endstepper %}
