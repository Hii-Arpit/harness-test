...
{% stepper %}
{% step %}
## Open the AutoStopping Proxy setup
...
* **Security Group**: Define the security rules that determine the inbound and outbound traffic.
...
* **Key Pair**: Enter the SSH key pair.
  * This key can be used to access the machine over SSH with the _ubuntu_ user.
* **API Key**: Enter the NG API key. Choose **No Expiration** in the Expiration dropdown list while creating this API key. Go to [Create an API Key](https://app.gitbook.com/s/3F2TpHXhur2QtQnORSM9/use-harness-platform/automation/api/api-quickstart) for more information.
...
- Select Cloud Provider as **Azure**. Select an existing Azure connector or create a new one.
...
Select the checkbox **Never ends** if you do not want to set end time for your schedule.
{% endstep %}
...
{% step %}
## Specify source and target ports
