...
* [Connectivity from target clusters and request authentication](index.md#connectivity-from-target-clusters-and-authentication-of-requests)
* [Kubernetes roles for the chaos infrastructure](index.md#kubernetes-roles-for-chaos-infrastructure)
* [User authentication to the Harness Chaos Engineering module](index.md#user-authentication)
* [Access control and permissions for chaos actions](index.md#user-authorization-and-role-based-access-control)
* [Secrets management](index.md#secrets-management), including [external secret manager support](index.md#use-an-external-secret-manager)
* [Blast radius control using permissions](index.md#blast-radius-control-using-permissions)
...
### Connectivity from target clusters and authentication of requests <a href="#connectivity-from-target-clusters-and-authentication-of-requests" id="connectivity-from-target-clusters-and-authentication-of-requests"></a>
...
### Kubernetes roles for chaos infrastructure <a href="#kubernetes-roles-for-chaos-infrastructure" id="kubernetes-roles-for-chaos-infrastructure"></a>

You can install the deployments that make up the chaos infrastructure with cluster-wide scope or namespace-only scope. These deployments are mapped to a dedicated service account that can execute all supported chaos experiments for that scope. To learn more about connecting to a chaos infrastructure in cluster or namespace mode, refer to [connect chaos infrastructures](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/use-chaos-engineering/infrastructure/types/types). Mapping deployments to dedicated service accounts is considered as the first level of blast radius control.

The permissions are listed below for reference.

{% hint style="info" %}
The permissions listed an be tuned for further minimization based on environments connected, type of experiments needed etc. Refer to [blast radius control using permissions](index.md#blast-radius-control-using-permissions) to learn more.
{% endhint %}
...
By default, control plane secrets are backed by the built-in Harness Secret Manager. You can also use an external secret manager to store and resolve these secrets. Go to [Use an external secret manager](index.md#use-an-external-secret-manager) to understand how this works.

### Use an external secret manager <a href="#use-an-external-secret-manager" id="use-an-external-secret-manager"></a>
...
This is useful when you already manage credentials in your own secret manager and you want chaos experiments to resolve secrets from the same source as your other Harness modules. Harness provides comprehensive setup guidance for every supported option. Go to [Secrets management](https://developer.harness.io/harness-platform/use-harness-platform/secrets/secrets-management) to review all supported secret managers.

How it works:

* Chaos experiments that run through [Harness Delegate](../chaos-testing/infrastructure/kubernetes/) use a dedicated chaos delegate task to decrypt secrets at runtime.
...
To set up a secret manager and add secrets, go to [Harness Secret Manager overview](https://developer.harness.io/harness-platform/use-harness-platform/secrets/secrets-management/harness-secret-manager-overview) to use the built-in option. To connect an external secret manager, go to [Add HashiCorp Vault](https://developer.harness.io/harness-platform/use-harness-platform/secrets/secrets-management/add-hashicorp-vault) or [Add a Google KMS secret manager](https://developer.harness.io/harness-platform/use-harness-platform/secrets/secrets-management/add-google-kms-secrets-manager).

### Secrets to access chaos artifact (Git) repositories <a href="#secrets-to-access-chaos-artifact-git-repositories" id="secrets-to-access-chaos-artifact-git-repositories"></a>

{% hint style="warning" %}
**GIT-BASED CHAOSHUBS DEPRECATED**

Git-based ChaosHubs have been removed. The Git connector and PAT/SSH key setup described below is no longer required for new deployments. Use [Templates](../chaos-testing/templates/) and [Resilience Probes](../chaos-testing/probes/) to manage reusable chaos artifacts instead.
{% endhint %}
...
![Control plane secrets](../.gitbook/assets/control-plane-secrets.png)

![Experiment secrets](../.gitbook/assets/experiment-secrets.png)

### Secrets to access and inject chaos on public and on-prem cloud resources <a href="#secrets-to-access-and-inject-chaos-on-public-and-on-prem-cloud-resources" id="secrets-to-access-and-inject-chaos-on-public-and-on-prem-cloud-resources"></a>
...
[Here](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/ec2-cpu-hog#prerequisites) is an example of AWS access information being fed to the experiment pods through a Kubernetes secret.

### Blast radius control using permissions <a href="#blast-radius-control-using-permissions" id="blast-radius-control-using-permissions"></a>
...
In the case of the Kubernetes chaos, a lower blast radius is achieved through service accounts mapped to custom roles instead of the default service accounts mentioned in the [Kubernetes roles for chaos infrastructure](index.md#kubernetes-roles-for-chaos-infrastructure). For non-Kubernetes chaos, a lower blast radius is achieved through cloud-specific role definitions (for example, IAM) mapped to the user account.

Every fault in the Enterprise chaos hub publishes the permissions that you need to execute the fault. You can tune your roles. Common permission templates that work as subsets or supersets for a specific category of experiments are also available. For example, [AWS resource faults](https://app.gitbook.com/s/lSkpbpeYJ3rfGUkIrcyQ/faults/chaos-fault-categories/aws/security-configurations/policy-for-all-aws-faults).

## Next steps <a href="#next-steps" id="next-steps"></a>

* [Openshift security policies](security-templates/openshift-scc.md)
* [Pod security policy](security-templates/psp.md)
* [Kyverno policy](security-templates/kyverno-policies.md)
