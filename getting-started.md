...
keywords:
  ...
  - pipeline yaml v1
  - migration
nodeTitle: What's New
inputFilePath: 3k-docs/platform/getting-started/index.md
originalUrl: https://developer.harness.io/3k-docs/platform/getting-started/index/
description: >-
  Understand what the Harness Platform is, what changes between Harness NextGen and
  Harness 3.0, and what those changes mean for your pipelines.
tags:
  - platform
  - getting-started
---
...
Harness 3.0 rebuilds that platform around a simplified pipeline format and a container-based execution model. Your existing Harness NextGen resources continue to work, but the way you author pipelines, run them, and navigate the UI changes.

This topic explains what the platform provides and compares Harness NextGen with Harness 3.0 so you know what to expect before you migrate.

***

### What the Harness Platform provides <a href="#what-the-harness-platform-provides" id="what-the-harness-platform-provides"></a>

Harness 3.0 ships as a single product. Delivery, security, runtime protection, and cost capabilities run on the same platform foundation and are grouped into [Harness Agents](getting-started.md#harness-agents) rather than separate products, so you no longer assemble a workflow out of pieces. When you configure authentication, permissions, or notifications at the platform level, those settings apply consistently across Harness.
...
### What changes in Harness 3.0 <a href="#what-changes-in-harness-30" id="what-changes-in-harness-30"></a>
...
* A single unified Delegate replaces the per-type Delegates of Harness NextGen.

These changes affect how you define pipelines, how you run them, and where you configure them.

#### What changes at a glance <a href="#what-changes-at-a-glance" id="what-changes-at-a-glance"></a>

The following table compares Harness NextGen with Harness 3.0 across multiple areas.

| Area             | Harness NextGen                                                                    | Harness 3.0                                                                                                     |
| ---------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
...
Agents are the largest addition in Harness 3.0. They replace the product-by-product boundaries of Harness NextGen with four agents that own a set of related capabilities, and they bring AI automation into your pipelines without giving up control, security, or governance.

#### The four Harness Agents <a href="#the-four-harness-agents" id="the-four-harness-agents"></a>
...
#### Why Agents matter <a href="#why-agents-matter" id="why-agents-matter"></a>
...
#### How an Agent runs <a href="#how-an-agent-runs" id="how-an-agent-runs"></a>
...
{% code title="pipeline-with-agent.yaml" %}
```yaml
pipeline:
  stages:
    - name: autofix-pr
...
For the full catalog, model connectors, input types, and governance controls, see [Harness Agents](agents/).

***

### Pipeline YAML format <a href="#pipeline-yaml-format" id="pipeline-yaml-format"></a>
...
{% code title="pipeline-v0.yaml (Harness NextGen)" %}
```yaml
pipeline:
...
{% code title="pipeline-v1.yaml (Harness 3.0)" %}
```yaml
pipeline:
  stages:
    - name: build
...
### Drone & GitHub Actions compatibility <a href="#drone-and-github-actions-compatibility" id="drone-and-github-actions-compatibility"></a>
...
### Stage groups <a href="#stage-groups" id="stage-groups"></a>
...
### No more fixed stage types <a href="#no-more-fixed-stage-types" id="no-more-fixed-stage-types"></a>

In Harness NextGen, stages had fixed types; a CI stage could only contain build steps, a CD stage could only contain deployment steps, and an Approval stage was its own separate construct. Harness 3.0 removes this restriction. Any stage can contain any combination of build steps, deployment steps, approval steps, and custom steps.

| Behavior            | Harness NextGen                                                   | Harness 3.0                                      |
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------ |
...
### Containerized step execution <a href="#containerized-step-execution" id="containerized-step-execution"></a>
...
{% code title="containerized-steps.yaml" %}
```yaml
pipeline:
  stages:
...
Existing `<+input>` runtime inputs from Harness NextGen pipelines will need to be converted to typed inputs when migrating to pipeline YAML v1. The Harness pipeline converter tool can assist with this migration.
{% endhint %}

***

### Versioned steps <a href="#versioned-steps" id="versioned-steps"></a>

In Harness NextGen, step implementations were bundled with the Delegate. Upgrading a Delegate upgraded all steps simultaneously, which could introduce breaking changes across all pipelines. In Harness 3.0, each step is independently versioned and shipped as its own container image. Teams can pin specific step versions per pipeline and upgrade on their own schedule.

| Aspect           | Harness NextGen                                               | Harness 3.0                                         |
| ---------------- | -------------------------------------------------------- | --------------------------------------------------- |
...
{% code title="versioned-steps.yaml" %}
```yaml
pipeline:
  stages:
    - name: build
...
### Redesigned Pipeline Studio <a href="#redesigned-pipeline-studio" id="redesigned-pipeline-studio"></a>

The Pipeline Studio in Harness 3.0 has been redesigned for speed and simplicity. The modal-based configuration flow from Harness NextGen has been replaced with a drawer pattern that keeps you in context while editing step details.
...
### Delegate 3.0 <a href="#delegate-30" id="delegate-30"></a>

Harness NextGen required different Delegate types depending on your infrastructure: a Kubernetes Delegate for K8s workloads, a Docker Delegate for container tasks, a Shell Delegate for VM-based operations, and so on. Harness 3.0 consolidates all of these into a single, unified, lightweight Delegate 3.0.
...
Existing Harness NextGen Delegates will continue to work during the transition period. Harness provides a migration tool to convert existing Delegate configurations to Delegate 3.0. See the Migration guide for detailed instructions.
{% endhint %}

***

### Redesigned navigation <a href="#redesigned-navigation" id="redesigned-navigation"></a>

In Harness NextGen, navigation was organized around separate products; you picked one (CI, CD, Feature Flags, and so on) and everything was scoped to it. Harness 3.0 removes those boundaries and replaces them with a holistic platform view where all capabilities are accessible from a single, unified navigation system.
...
For a detailed breakdown of the new navigation system, see [Navigation](navigation/).
...
The Harness AI Assistant in 3.0 is a significant upgrade over the AIDA floating button in Harness NextGen. It is integrated directly into the navigation panel instead of appearing as a floating overlay, and it can perform actions, not just answer questions.
...
* [Harness Agents](agents/): Agent catalog, model connectors, and governance controls.
* Backward Compatibility: Harness NextGen resources that keep working in Harness 3.0.
* Migration: Move your pipelines to YAML v1.
* [Navigation](navigation/): The redesigned navigation in detail.
